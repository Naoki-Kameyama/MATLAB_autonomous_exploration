    ...
    if mod(m,3) == 0 || m == 1  
        ...
        if name == 'o' && (abs(tfdata.Transforms.Transform.Translation.X) > 0 || abs(tfdata.Transforms.Transform.Translation.Y) > 0 || abs(tfdata.Transforms.Transform.Translation.Z) > 0);
            q=q+1;
                      ...
                      if q > 10%%switch判断
                        swipub = rospublisher('switch','std_msgs/Bool');
                        swimsg = rosmessage(swipub);
                        swimsg.Data = true;
                        send(swipub,swimsg);
                        while true
                            if mod(m,3) == 0 || m == 1  
                                ...
                                figure(1)
                                scatter(ccodomx(p,1),ccodomy(p,1),'d')
                                hold on
                                odom = getOdometry(tbot);
                                p = p + 1;
                            end
...
%% target detection decision
    ...
    if isempty(x) == 0 && isempty(y) == 0
        ...
        % nu
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        unknown = ydis(abs(ydis)>1.0); %1.5m
        findmat = find(abs(ydis)>1.0); %1.5m
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        odom = getOdometry(tbot);
    ...
    %% VFH 
            ...
            if  S == 0
                ...
                while toc < 100
                    ...
                    laserScan.Ranges(isnan(laserScan.Ranges(:,1))) = 0;
                    S = sum(laserScan.Ranges(:,1));
                    if  S > 400    %if laser scan data is received, break
                        break
                    end
                end
                pause(1.5)
